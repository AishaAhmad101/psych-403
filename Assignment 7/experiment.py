...
#-frame-rate timing
refresh=1.0/60.0
frame_timing = stimulus_duration / refresh
...
for block in range(blocks):
    ...
    for trial in range(trials):
        ...
        for frameN in range(int(frame_timing*3)): #for the whole trial...
            #-draw stimulus
            if 0 <= frameN <= frame_timing: #number of frames for fixation      
                # -draw fixation
                fix_text.draw()
                # -flip window
                win.flip()
            ...
            if frame_timing < frameN <= (frame_timing+frame_timing):      
                my_image.draw() #draw
                win.flip() #show 
                    
            #number of frames for the final text stimulus    
            if (frame_timing+frame_timing) < frameN < frame_timing*3:  
                end_trial_text.draw() #draw
                win.flip() #show  
                 
        print('Overall, %i frames were dropped.' % win.nDroppedFrames)

    # print(block_timer.getTime())             
